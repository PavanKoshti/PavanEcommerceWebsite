              ...
              <form>
                <div class="a-spacing-medium a-spacing-top-medium">
                  ...
                  <div class="a-spacing-top-medium">
                    ...
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="streetAddress1" />
                    <!-- Street Address 2 -->
                    <input type="text" class="a-input-text a-spacing-top-small" style="width: 100%;"
                      v-model="streetAddress2" placeholder="Apartment, suite, unit, building, floor, etc." />
                  </div>
                  <!-- City -->
                  <div class="a-spacing-top-medium">
                  ...
                  <div class="a-spacing-top-medium">
                    ...
                    <textarea style="height:6em; width: 100%;" v-model="deliveryInstructions"></textarea>
                  </div>
                  <!-- Security code -->
                  <div class="a-spacing-top-medium">
                    <label style="margin-bottom: 0px;">Do we need a security code or a call box number to access this
                      building?</label>
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="securityCode" />
                  </div>
                  <div class="a-spacing-top-medium">
...
export default {
  ...
  created: async function () {
    ...
    await this.getsingleAddressList();
  },
    ...
    async getCountryList() {
      try {
        ...
        if (countries.success == true) {
          ...
          console.log("Error while Get Country List >>>");
        }
      } catch (error) {
        console.log("Error while Get Country List Catch >>>", error);
      }

    },

    async getsingleAddressList() {
      try {
        let singleAddress = await this.$axios.$get(`api/addresses/${this.$route.params.id}`);
        if (singleAddress.success == true) {
          this.country = singleAddress.address.country;
          this.fullName = singleAddress.address.fullName
          this.streetAddress1 = singleAddress.address.streetAddress
          this.city = singleAddress.address.city
          this.state = singleAddress.address.state
          this.zipCode = singleAddress.address.zipCode
          this.phoneNumber = singleAddress.address.phoneNumber
          this.deliveryInstructions = singleAddress.address.deliverInstructions
          this.securityCode = singleAddress.address.securityCode
        }
        else {
          console.log("Error While Get Single Address Details");
        }
      } catch (error) {
        console.log("Error While Get Single Address Details Catch >>>", error);
      }
