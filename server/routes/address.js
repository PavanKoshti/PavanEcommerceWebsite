...
router.post("/addresses", verifyToken, async (req, res) => {
    try {
        ...
        console.log("Error While get Add address >>>", err);
        res.status(500).json({
            success: false,
            message: err.message
        });
...
router.get("/addresses", verifyToken, async (req, res) => {
    try {
        ...
        console.log("Error While get All address list >>>", err);
        res.status(500).json({
            success: false,
            message: err.message
        });
...
router.get("/countries", async (req, res) => {
    try {
        ...
        console.log("Error While Get All Cointries >>>", err);
        res.status(500).json({
            success: false,
            message: err.message
        });
...
router.delete("/addresses/:id", verifyToken, async (req, res) => {
    try {
        ...
        console.log("Error While Delete Address >>>", err);
        res.status(500).json({
            success: false,
            message: err.message
        });
...
router.put("/addresses/set/default", verifyToken, async (req, res) => {
    try {
        ...
        console.log("Error While Get Set Default Address >>>",err);
        res.status(500).json({
            success: false,
            message: err.message
        });
